...
from app.domain.inventory.heuristic import (
...
class InventorySlotFillingAgent(SlotFillingAgent):
    ...
    async def extract(self, context: AgentContext) -> SlotExtractionResult:
        ...
        try:
            ...
            LOGGER.exception("Inventory LLM extraction failed, using heuristic fallback")

        if heuristic_conf < LOW_CONFIDENCE_THRESHOLD:
            LOGGER.warning("Low-confidence inventory extraction", extra={"heuristic_confidence": heuristic_conf})
